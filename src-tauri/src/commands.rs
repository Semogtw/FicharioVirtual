...
use crate::{catalog, paths, storage};
...
pub fn native_status(app: AppHandle) -> Result<NativeStatus, String> {
    ...
    let sync_jobs = catalog::list_sync_jobs(&paths, 100)?;
    Ok(NativeStatus {
        platform: std::env::consts::OS.to_string(),
        schema_version: 1,
        local_document_count: documents
            .iter()
            .filter(|document| document.local_state == "present")
            .count(),
        pending_sync_count: sync_jobs.len(),
        disk_usage_bytes: storage::disk_usage(&paths)?,
...
pub fn native_disk_usage(app: AppHandle) -> Result<u64, String> {
    storage::disk_usage(&app_paths(&app)?)
}
