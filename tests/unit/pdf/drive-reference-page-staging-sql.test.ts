...
describe('Drive PDF descriptor staging migration', () => {
	it('stores page descriptors under the durable reference with owner RLS', () => {
		...
		expect(source).toContain(
			'references public.drive_pdf_reference_imports(document_id) on delete cascade'
		);
	...
	it('validates exact six-key payloads and uses immutable retry semantics', () => {
		...
		expect(source).toContain(
			"descriptor ?& array['id', 'pageNumber', 'nativeText', 'needsOcr', 'temporaryImagePath', 'jobId']"
		);
	...
	it('keeps direct writes unavailable to authenticated clients and exposes only the RPC', () => {
		...
		expect(source).toContain(
			'revoke execute on function public.stage_drive_pdf_reference_page_batch'
		);
		expect(source).toContain(
			'grant execute on function public.stage_drive_pdf_reference_page_batch'
		);
	});
});
