...
function secureHeaders(overrides: Record<string, string> = {}) {
	return new Headers({
		'content-security-policy':
			"default-src 'self'; base-uri 'self'; form-action 'self'; object-src 'none'; frame-ancestors 'none'; script-src 'self' 'wasm-unsafe-eval' https://apis.google.com; connect-src 'self' https://*.supabase.co wss://*.supabase.co https://www.googleapis.com",
	...
	it('rejects permissive CSP script policies and weak HSTS', () => {
		expect(() =>
			assertSecurityHeaders(
				secureHeaders({
					'content-security-policy':
						"default-src 'self'; base-uri 'self'; form-action 'self'; object-src 'none'; frame-ancestors 'none'; script-src 'self' 'wasm-unsafe-eval' https://apis.google.com 'unsafe-inline'; connect-src 'self' https://*.supabase.co wss://*.supabase.co https://www.googleapis.com"
				})
			)
