...
describe('real deployed app flow workflow', () => {
	...
	it('fetches the exact deployed SHA and uses pinned browser dependencies', () => {
		...
		expect(source).toContain('check-real-app-special-routes.mjs');
	...
	it('runs core, action, exhaustive and special-route flows independently and rejects any failure', () => {
		...
		expect(source).toContain('node check-real-app-special-routes.mjs');
		expect(source).toContain(
		...
		expect(source).toContain('REAL_APP_SPECIAL_REPORT_PATH: /tmp/real-app-special-report.json');
		expect(source).toContain('REAL_APP_SPECIAL_EVIDENCE_DIR: /tmp/real-app-special-evidence');
		...
		expect(source).toContain('SPECIAL_OUTCOME: ${{ steps.special-routes.outcome }}');
		...
		expect(source).toContain('test "$SPECIAL_OUTCOME" = success');
	});
});
