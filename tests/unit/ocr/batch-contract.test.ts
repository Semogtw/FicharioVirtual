...
function result(page = first, text = 'Texto', overrides: Record<string, unknown> = {}) {
	return {
		...page,
		text,
...
describe('parseOcrBatchPayload', () => {
	...
	it('derives word boxes locally from one compact provider box per non-empty line', () => {
		...
		expect(parsed.pages[0]?.wordGeometry[3]).toEqual(
			expect.arrayContaining(['Segunda', 3000, 3600])
		);
	});
