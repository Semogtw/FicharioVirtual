...
const coverage = readFileSync('supabase/functions/semantic-coverage/index.ts', 'utf8');
...
describe('semantic provider telemetry contract', () => {
	...
	it('records document indexing and query embeddings as separate operations', () => {
		expect(coverage).toContain("operation: 'document_embedding'");
		expect(coverage).toContain("taskType: 'RETRIEVAL_DOCUMENT'");
		expect(coverage).toContain("operation: 'query_embedding'");
		expect(coverage).toContain("taskType: 'RETRIEVAL_QUERY'");
		expect(coverage.match(/requestGeminiEmbeddingsWithTelemetry\(/g)).toHaveLength(2);
	});
