...
describe('Drive sync Edge Function boundary', () => {
	it('authenticates the Fichário user and obtains credentials only through service RPCs', () => {
		...
		expect(source).toContain("rpc('begin_drive_remote_sync'");
	...
	it('applies every change before advancing its compare-and-swap checkpoint', () => {
		const source = readFileSync(path, 'utf8');
		const fetchIndex = source.indexOf('listGoogleDriveChanges');
		const applyIndex = source.indexOf("rpc('apply_drive_remote_change'");
		const checkpointIndex = source.indexOf("rpc('persist_drive_change_checkpoint'");
	...
	it('persists a sanitized resumable failure without exposing access tokens', () => {
		...
		expect(source).toContain("rpc('fail_drive_remote_sync'");
