...
describe('Drive job runner Edge Function boundary', () => {
	it('authenticates the Fichário user and claims leased jobs through service-only RPCs', () => {
		...
		expect(source).toMatch(rpc('claim_drive_sync_job_for_user'));
	...
	it('uses strict Drive helpers and all three terminal job transitions', () => {
		...
		expect(source).toMatch(rpc('complete_drive_sync_job'));
		expect(source).toMatch(rpc('retry_drive_sync_job'));
		expect(source).toMatch(rpc('conflict_drive_sync_job'));
	});
