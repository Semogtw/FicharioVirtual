...
describe('draft route local fallback', () => {
	...
	it('does not claim an unresolved page was deleted', () => {
		expect(source).toMatch(
			/row\.location\?\.documentTitle\s*\?\?\s*\(locationsReady\s*\?\s*'Página não encontrada no servidor'\s*:\s*'Localização indisponível'\)/
		);
		expect(source).toContain('{:else if locationsReady}');
