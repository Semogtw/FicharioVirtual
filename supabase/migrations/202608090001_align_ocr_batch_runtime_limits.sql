-- Keep database manifests aligned with the application/provider batch ceiling.
-- NOT VALID avoids blocking rollout on historical rows while enforcing the new
-- limit for every insert/update performed after this migration.

alter table public.ocr_batches
  drop constraint if exists ocr_batches_page_ids_check;

alter table public.ocr_batches
  add constraint ocr_batches_page_ids_check
  check (cardinality(page_ids) between 1 and 100) not valid;
...
begin
  if current_user_id is null or attempted_pages < 1 or attempted_pages > 100 then
    return false;
  end if;
