-- Restore the active-user authorization guard on the privileged OCR quota blocker.
-- The Pacific-time RPD reset migration replaced this SECURITY DEFINER function and
-- accidentally dropped the allowlist check that protects its elevated usage writes.
-- Keep the Pacific reset behavior while restoring the existing security boundary.
