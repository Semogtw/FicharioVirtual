...
-- Reuse the existing user-scoped OCR transaction boundaries without granting
-- the worker broad ad-hoc write access. Only service_role may invoke this
-- dispatcher, and nested RPCs continue to scope rows through auth.uid().
