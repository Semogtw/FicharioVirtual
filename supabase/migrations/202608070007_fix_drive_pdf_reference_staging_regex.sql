...
begin
  ...
  where document.id = new.document_id
    ...
    and char_length(document.drive_file_id) between 10 and 256
    and document.drive_file_id ~ '^[A-Za-z0-9_-]+$'
    and char_length(document.drive_parent_folder_id) between 10 and 256
    and document.drive_parent_folder_id ~ '^[A-Za-z0-9_-]+$'
    and document.drive_mime_type = 'application/pdf'
    and document.drive_modified_time is not null
    and char_length(document.drive_version) between 1 and 32
    and document.drive_version ~ '^[0-9]+$'
    and (
      document.drive_md5_checksum is null
      or (
        char_length(document.drive_md5_checksum) = 32
        and document.drive_md5_checksum ~ '^[0-9a-fA-F]+$'
      )
    );
