create or replace function public.create_pdf_import(
...
begin
  ...
  then
    raise exception 'invalid PDF metadata' using errcode = '22023';
  end if;
  if original_storage_path not like current_user_id::text || '/%'
    or original_storage_path !~ '^[A-Za-z0-9_/-]+\.pdf$'
  then
    raise exception 'invalid PDF storage ownership' using errcode = '42501';
  end if;
...
revoke execute on function public.create_pdf_import(
  uuid, uuid, text, text, text, text, timestamptz, jsonb, integer
) from public, anon;
grant execute on function public.create_pdf_import(
  uuid, uuid, text, text, text, text, timestamptz, jsonb, integer
) to authenticated;
