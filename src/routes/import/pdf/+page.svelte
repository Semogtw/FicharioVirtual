<script lang="ts">
	...
	const labels = {
		...
		reading: 'Lendo páginas selecionadas',
...
<svelte:head>
	<title>Importar PDFs — Fichário Virtual</title>
</svelte:head>
...
<div class="page" aria-labelledby="page-title">
	<header>
		<div>
			...
			<p>
				O texto nativo fica local e pesquisável. A leitura externa recebe somente as páginas sem
	...
	>
		...
		<p>Até 20 MB por arquivo. A inspeção acontece em um worker local.</p>
		<label class="file-button">
			Selecionar PDFs
			<input type="file" accept="application/pdf" multiple onchange={selected} />
		</label>
	</section>
				...
				{#each pdfImportQueue.items as item (item.id)}
					<li>
						...
						<div class="actions">
							...
							{/if}
							{#if item.status === 'duplicate' && item.duplicateDocumentId}
								<a href={`/documents/${item.duplicateDocumentId}/`}>Abrir existente</a>
							{/if}
							{#if ['complete', 'needs_review', 'duplicate'].includes(item.status)}
								<button type="button" onclick={() => removePdfImport(item.id)}>Ocultar</button>
							{/if}
						</div>
					</li>
				{/each}
