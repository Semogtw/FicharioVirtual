<script lang="ts">
	...
	function canOpen(item: PdfQueueItem) {
		return (
			['complete', 'needs_review', 'waiting', 'failed'].includes(item.status) && item.result !== null
		);
	}
...
<div class="page" aria-labelledby="page-title">
	...
	<section class="options" aria-label="Opções do PDF">
		<label>
			<span>Caderno</span>
			<select bind:value={notebookId} disabled={notebookLoading || !notebookOptionsReady} onchange={selectNotebook}>
	...
	{#if notebookError}
		<div class="notebook-warning" role="status">
			<p>{notebookError}</p><button type="button" onclick={() => void loadNotebookOptions()}>Tentar novamente</button>
		</div>
	...
	>
		...
		<label class="file-button">Selecionar PDFs<input type="file" accept="application/pdf" multiple onchange={selected} /></label>
	</section>
	...
	{#if pdfImportQueue.items.length > 0}
		<section class="queue" aria-labelledby="queue-title">
			<div class="queue-heading">
				<div><p class="eyebrow">Fila sequencial</p><h2 id="queue-title">PDFs</h2></div>
				...
				{#each pdfImportQueue.items as item (item.id)}
					<li>
						...
						<div class="item-copy">
							...
							{#if item.progress}
								<small>{item.progress.completed}/{item.progress.total}{#if item.progress.pageNumber} · página {item.progress.pageNumber}{/if}</small>
							{/if}
							{#if item.result}
								<small>
									{item.result.pageCount} páginas · {item.result.ocrPageCount} encaminhadas para OCR
									{#if item.result.ocrFailed > 0} · {item.result.ocrFailed} falharam{/if}
								</small>
							{/if}
						...
						<div class="actions">
							...
							{/if}
							{#if canOpen(item) && item.result}<a href={`/documents/${item.result.documentId}/`}>{item.status === 'needs_review' ? 'Revisar' : 'Abrir'}</a>{/if}
							{#if item.status === 'duplicate' && item.duplicateDocumentId}<a href={`/documents/${item.duplicateDocumentId}/`}>Abrir existente</a>{/if}
							{#if ['complete', 'needs_review', 'duplicate'].includes(item.status)}<button type="button" onclick={() => removePdfImport(item.id)}>Ocultar</button>{/if}
						</div>
					</li>
				{/each}
			</ul>
...
<style>
	.page, .queue { display: grid; gap: 1.35rem; }
	header, .queue-heading { display: flex; align-items: end; justify-content: space-between; gap: 1.5rem; }
	.eyebrow { margin-bottom: .45rem; color: var(--archive); font-size: .75rem; font-weight: 780; letter-spacing: .12em; text-transform: uppercase; }
	h1, h2 { font-family: var(--font-heading); font-weight: 520; }
	h1 { margin-bottom: .55rem; font-size: clamp(2.4rem, 6vw, 4.5rem); letter-spacing: -.04em; }
	header p:last-child { max-width: 52rem; margin-bottom: 0; color: var(--muted); line-height: 1.6; }
	.options { display: grid; grid-template-columns: minmax(12rem,.4fr) minmax(22rem,1fr); gap: 1rem; padding: 1rem; border: 1px solid var(--line); border-radius: var(--radius-md); background: var(--surface); }
	.options > label:first-child { display: grid; gap: .4rem; }
	.options label > span:first-child { color: var(--muted); font-size: .75rem; font-weight: 740; }
	select { min-height: 3rem; padding: .65rem .75rem; border: 1px solid var(--line-strong); border-radius: var(--radius-sm); background: var(--surface-strong); color: var(--ink); }
	.consent { display: flex; align-items: flex-start; gap: .7rem; padding: .8rem; border-left: .3rem solid var(--accent); background: rgb(166 94 67 / 7%); }
	.consent input { width: 1.1rem; height: 1.1rem; margin-top: .2rem; }
	.consent span, .item-copy { display: grid; gap: .25rem; }
	.consent small, .queue-heading > span, .item-copy small { color: var(--muted); }
	.drop-zone { display: grid; justify-items: center; padding: clamp(2rem,7vw,4.5rem); border: .125rem dashed var(--line-strong); border-radius: var(--radius-lg); background: var(--surface); text-align: center; }
	.drop-zone.dragging { border-color: var(--archive); background: var(--archive-soft); }
	.pdf-mark, .file-icon { display: grid; place-items: center; background: var(--accent); color: white; font-size: .7rem; font-weight: 800; letter-spacing: .08em; }
	.pdf-mark { width: 4rem; height: 4.5rem; margin-bottom: 1rem; border-radius: .35rem; box-shadow: var(--shadow-soft); }
	.drop-zone h2 { margin-bottom: .4rem; font-size: clamp(1.5rem,4vw,2.25rem); }
	.drop-zone p { max-width: 48rem; margin-bottom: 1.25rem; color: var(--muted); }
	.file-button, .actions button, .actions a { min-height: 2.5rem; display: inline-flex; align-items: center; padding: .55rem .8rem; border-radius: var(--radius-sm); font-weight: 740; cursor: pointer; }
	.file-button { position: relative; background: var(--archive); color: white; }
	.file-button input { position: absolute; width: 1px; height: 1px; opacity: 0; }
	.notebook-warning, .error { padding: .8rem 1rem; border-left: .3rem solid var(--accent); background: rgb(166 94 67 / 7%); }
	.notebook-warning { display: flex; align-items: center; justify-content: space-between; gap: 1rem; }
	.notebook-warning p, .error { margin: 0; }
	.error { border-color: var(--danger); color: var(--danger); }
	.notebook-warning button, .actions button, .actions a { border: 1px solid var(--line-strong); background: var(--surface-strong); color: var(--ink); }
	.queue-heading h2 { margin-bottom: 0; font-size: 2rem; }
	ul { display: grid; gap: .6rem; margin: 0; padding: 0; list-style: none; }
	li { display: grid; grid-template-columns: 3.5rem minmax(0,1fr) auto; align-items: center; gap: .9rem; padding: .75rem; border: 1px solid var(--line); border-radius: var(--radius-md); background: var(--surface); }
	.file-icon { width: 3.5rem; height: 4rem; border-radius: .3rem; }
	.item-copy { min-width: 0; }
	.item-copy > strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
	.status { width: fit-content; color: var(--archive); font-size: .75rem; font-weight: 740; }
	.status.waiting, .status.needs_review, .status.duplicate { color: var(--accent-strong); }
	.status.failed, .status.cancelled, .item-copy p { color: var(--danger); }
	.item-copy p { margin: .15rem 0 0; font-size: .78rem; }
	.actions { display: flex; flex-wrap: wrap; justify-content: flex-end; gap: .4rem; }
	.actions button, .actions a { min-height: 2.35rem; padding: .5rem .7rem; font-size: .78rem; }
	@media (max-width:760px) { .options { grid-template-columns: 1fr; } }
	@media (max-width:600px) { header { align-items: flex-start; flex-direction: column; } li { grid-template-columns: 3.5rem minmax(0,1fr); } .actions { grid-column: 1/-1; justify-content: flex-start; } }
</style>
