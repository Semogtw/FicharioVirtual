<script lang="ts">
	...
	let createdTo = $state('');
	let hasActiveFilters = $derived(Boolean(notebookId || kind || status || createdFrom || createdTo));
...
<div class="page" aria-labelledby="page-title">
	...
	<form class="filters" aria-label="Filtrar biblioteca" onsubmit={(event) => event.preventDefault()}>
		<label>
			<span>Caderno</span>
			<NativeSelect ariaLabel="Caderno" bind:value={notebookId} disabled={notebookLoading} onchange={() => void load(true)}>
	...
	{#if notebookError}
		<div class="filter-warning" role="status">
			<p>{notebookError}</p>
			<Button label="Tentar carregar cadernos" variant="secondary" onclick={() => void loadNotebookOptions()} />
		</div>
	{/if}
	...
	{:else if documents.length === 0}
		<EmptyState
			...
			description={hasActiveFilters ? 'Limpe ou ajuste os filtros para voltar a ver outros documentos.' : 'Importe uma imagem ou PDF para começar sua biblioteca.'}
	...
	{:else}
		...
		{#if nextCursor}
			<div class="load-more">
				<Button label={loadingMore ? 'Carregando…' : 'Carregar mais'} variant="secondary" disabled={loadingMore} onclick={() => void load(false)} />
			</div>
		{/if}
	{/if}
</div>
...
<style>
	...
	.page-header { display: flex; align-items: end; justify-content: space-between; gap: 1.5rem; }
	.eyebrow { margin-bottom: 0.45rem; color: var(--archive); font-size: 0.75rem; font-weight: 780; letter-spacing: 0.12em; text-transform: uppercase; }
	h1 { margin-bottom: 0.55rem; font-family: var(--font-heading); font-size: clamp(2.4rem, 6vw, 4.5rem); font-weight: 520; letter-spacing: -0.04em; }
	.page-header p:last-child { margin-bottom: 0; color: var(--muted); }
	.primary-action { min-height: 2.8rem; display: inline-flex; align-items: center; padding: 0.7rem 1rem; border-radius: var(--radius-sm); background: var(--archive); color: white; font-weight: 740; }
	.filters { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 0.75rem; padding: 1rem; border: 1px solid var(--line); border-radius: var(--radius-md); background: var(--surface); }
	label { display: grid; gap: 0.35rem; }
	label span { color: var(--muted); font-size: 0.72rem; font-weight: 740; }
	input { width: 100%; min-height: 2.7rem; padding: 0.55rem 0.65rem; border: 1px solid var(--line-strong); border-radius: var(--radius-sm); background: var(--surface-strong); color: var(--ink); }
	.filter-actions { grid-column: 1 / -1; display: flex; align-items: center; justify-content: space-between; gap: 0.75rem; padding-top: 0.15rem; border-top: 1px solid var(--line); color: var(--muted); font-size: 0.78rem; }
	.grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(14rem, 1fr)); gap: 1rem; }
	.grid :global(.document-card) { content-visibility: auto; contain-intrinsic-size: auto 20rem; }
	.filter-warning, .error { display: flex; align-items: center; justify-content: space-between; gap: 1rem; padding: 1rem; border-left: 0.3rem solid var(--danger); background: rgb(155 63 54 / 7%); }
	.filter-warning { border-left: 0.3rem solid var(--accent); background: rgb(166 94 67 / 7%); }
	.filter-warning p, .error p { margin: 0; }
	.filter-warning p { color: var(--accent-strong); }
	.error p { color: var(--danger); }
	.load-more { display: flex; justify-content: center; padding-top: 0.5rem; }
	@media (max-width: 900px) { .filters { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
	@media (max-width: 560px) { .page-header { align-items: flex-start; flex-direction: column; } .filters { grid-template-columns: 1fr; } }
</style>
