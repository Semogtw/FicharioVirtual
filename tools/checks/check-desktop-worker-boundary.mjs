...
const [helper, contract, pair, worker, config, authMigration, sourceBindingMigration, completionMigration] =
	await Promise.all(
		[
			helperPath,
			contractPath,
...
requireSource(contract, /action:\s*'complete'/, 'worker contract must define the completion action');
requireSource(contract, /sourceSha256:\s*string/, 'completion contract must carry the exact source digest');
requireSource(contract, /backend:\s*'transformers'\s*\|\s*'ollama'/, 'completion contract must allow only approved local backends');
requireSource(contract, /MAX_TEXT_LENGTH\s*=\s*1_000_000\b/, 'completion text must retain its 1 MB ceiling');
requireSource(contract, /MAX_WARNINGS\s*=\s*100\b/, 'completion warnings must retain their cardinality ceiling');
...
requireSource(worker, /parseDesktopWorkerRequest\(rawBody\)/, 'worker endpoint must use the strict shared request contract');
...
requireSource(worker, /bind_desktop_ocr_job_source_hash/, 'worker endpoint must bind source integrity before delivery');
requireSource(worker, /complete_desktop_ocr_job/, 'worker endpoint must complete through the lease-bound service RPC');
requireSource(worker, /clear_desktop_ocr_completed_source/, 'worker endpoint must clear the source pointer only after storage cleanup');
requireSource(worker, /\.remove\(\[completion\.sourceStoragePath\]\)/, 'worker completion must delete the private derivative before clearing its pointer');
requireSource(worker, /cleanupPending/, 'worker completion must expose recoverable derivative cleanup state');
...
requireSource(sourceBindingMigration, /desktop_source_sha256\s+text/, 'active desktop leases must persist the delivered source digest');
requireSource(sourceBindingMigration, /bind_desktop_ocr_job_source_hash/, 'source digest binding RPC must remain present');
requireSource(
...
requireSource(completionMigration, /provider,\s*model,\s*raw_text,/s, 'desktop completion must persist immutable OCR result provenance');
requireSource(completionMigration, /'local',\s*target_model,/s, 'desktop completion result provider must be local');
requireSource(completionMigration, /current_job\.desktop_source_sha256\s+is distinct from\s+target_source_sha256/, 'desktop completion must require the bound source digest');
requireSource(completionMigration, /current_job\.desktop_lease_id\s+is distinct from\s+target_lease_id/, 'desktop completion must require the exact lease id');
requireSource(completionMigration, /current_job\.desktop_lease_device_id\s+is distinct from\s+target_device_id/, 'desktop completion must require the exact device id');
requireSource(completionMigration, /'source',\s*'desktop_worker'/, 'local OCR result metadata must identify the desktop worker source');
requireSource(completionMigration, /'desktopLeaseId',\s*target_lease_id/, 'local OCR result metadata must preserve lease provenance for retries');
requireSource(completionMigration, /'sourceSha256',\s*target_source_sha256/, 'local OCR result metadata must preserve source integrity provenance');
requireSource(completionMigration, /clear_desktop_ocr_completed_source/, 'completion migration must provide constrained derivative cleanup');
requireSource(
	completionMigration,
