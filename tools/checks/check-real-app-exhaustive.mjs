...
const reportPath = process.env.REAL_APP_EXHAUSTIVE_REPORT_PATH ?? '/tmp/real-app-exhaustive-report.json';
const evidenceDir = process.env.REAL_APP_EXHAUSTIVE_EVIDENCE_DIR ?? '/tmp/real-app-exhaustive-evidence';
  ...
  blocked: [
    {
      ...
      reason: 'A UI do Google exige uma sessão Google/consentimento interativo externo ao navegador autenticado do CI; contratos, bootstrap OAuth e backend Drive são verificados por outros gates.'
    },
    {
      feature: 'Worker OCR em hardware físico',
      reason: 'O runner não possui o computador do usuário, Secret Service/Ollama/GPU reais; pareamento, autenticação, leases, spool, engine e protocolo são cobertos pelos gates de staging/unitários.'
    }
  ]
};
...
try {
  ...
  page.on('response', (response) => {
    if (response.status() >= 500) {
      ...
      if (url.origin === target.origin || url.origin === new URL(supabaseUrl).origin) {
        report.browser.serverErrors.push(`${response.status()} ${url.origin}${url.pathname}`.slice(0, 500));
      }
    }
  });
  ...
  for (const [route, heading] of [
    ...
    ['/drive/conflicts/', 'Conflitos do Drive'],
    ['/drive/jobs/', 'Trabalhos do Drive'],
    ['/settings/computers/', 'Computadores'],
    ['/settings/computers/queue/', 'Fila de OCR local']
  ]) {
  ...
  const destinationNotebook = await waitForRow(client, 'notebooks', { name: destinationNotebookName });
  ...
  await page.getByText(/arquivo\(s\) adicionados à fila global/i).waitFor({ state: 'visible', timeout: 25_000 });
  ...
  await page.getByText(imported.title, { exact: true }).first().waitFor({ state: 'visible', timeout: 20_000 });
  await page.getByLabel('De', { exact: true }).fill('2099-01-01');
  await page.getByText('Nenhum documento neste recorte', { exact: true }).waitFor({ state: 'visible', timeout: 20_000 });
  ...
  const organizationRow = page.locator('section.documents article').filter({ hasText: imported.title }).first();
  ...
  await organizationRow.getByText('Salvo', { exact: true }).waitFor({ state: 'visible', timeout: 20_000 });
  ...
  await page.getByText('Tag criada.', { exact: true }).waitFor({ state: 'visible', timeout: 20_000 });
  const documentMembershipRow = page.locator('ul.documents li').filter({ hasText: organizedTitle }).first();
  ...
  if (!createdTag || createdTag.document_count !== 1) throw new Error('Tag membership was not persisted');
  page.once('dialog', (dialog) => dialog.accept(renamedTagName));
  await page.getByRole('button', { name: 'Renomear', exact: true }).click();
  await page.getByText('Tag renomeada.', { exact: true }).waitFor({ state: 'visible', timeout: 20_000 });
  await page.getByText(renamedTagName, { exact: true }).first().waitFor({ state: 'visible', timeout: 20_000 });
  page.once('dialog', (dialog) => dialog.accept());
  await page.getByRole('button', { name: 'Excluir', exact: true }).click();
  await page.getByText('Tag excluída.', { exact: true }).waitFor({ state: 'visible', timeout: 20_000 });
  ...
  const fuzzyResult = page.locator('section.results li').filter({ hasText: organizedTitle }).first();
  await fuzzyResult.waitFor({ state: 'visible', timeout: 45_000 });
  await fuzzyResult.locator('a').click();
  await page.getByText(/Aberto a partir da busca por/).waitFor({ state: 'visible', timeout: 20_000 });
  ...
  if (!/^\S{4,}$/.test(pairingText.trim())) throw new Error('Pairing code UI did not expose a valid one-time code');
  stage('desktop-ocr-management-ui', 'pass', 'Código de uso único criado e exibido sem registrá-lo na evidência.');
  ...
  await page.reload({ waitUntil: 'domcontentloaded', timeout: 30_000 });
  await page.getByRole('heading', { name: 'Configurações', exact: true }).waitFor({ state: 'visible', timeout: 20_000 });
